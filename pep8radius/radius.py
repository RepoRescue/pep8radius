...
def fix_line_range(source_code, start, end, options):
    ...
    try:
        if options.docformatter:
            # docformatter >= 1.4 changed API from format_code function to Formatter class
            try:
                ...
                # docformatter >= 1.4 uses Formatter class
                from docformatter import Formatter
                import argparse

                # Create args for Formatter
                formatter_args = argparse.Namespace(
                    line_length=options.max_line_length,
                    wrap_summaries=options.max_line_length - 1,
                    wrap_descriptions=options.max_line_length - 2 * options.indent_size,
                    make_summary_multi_line=False,
                    force_wrap=options.force_wrap,
                    tab_width=options.indent_size,
                    pre_summary_newline=options.pre_summary_newline,
                    split_before_logical_operator=True,
                    indent_range=[start, end],
                    docstring_length=None,
                    length_range=(10, 10000),
                    make_one_line_summary=False,
                    close_quotes_on_newline=False,
                    pre_summary_multi_line_newline=options.pre_summary_newline,
                    pre_summary_new_line=options.pre_summary_newline,
                    capsule_word=None,
                    black=False,
                    style=None,
                    non_strict=False,
                    non_blank=not options.post_description_blank,
                    quiet=0
                )

                formatter = Formatter(formatter_args, '', '', '')
                fixed = formatter._format_code(fixed)
